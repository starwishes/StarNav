...
export const normalizeId = (id: unknown): number => {
  const num = Number(id)
  return Number.isNaN(num) ? 0 : num
}

type NormalizeInput = unknown

// 递归清洗对象中的 ID
export const normalizeData = (data: NormalizeInput): NormalizeInput => {
  if (!data) return data
  ...
  if (typeof data === 'object') {
    const newData: Record<string, unknown> = { ...(data as Record<string, unknown>) }
