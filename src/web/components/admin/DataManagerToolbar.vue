<template>
  <div class="toolbar-card">
    <div class="content-tabs" role="tablist" :aria-label="t('manage.tabsLabel')">
      <button
        type="button"
        role="tab"
        class="tab-button"
...
const emit = defineEmits<{
  ...
  (e: 'show-bookmark-import'): void
  (e: 'clean-duplicates'): void
  (e: 'json-import', backup: ParsedJsonBackup): void
}>()
...
const handleExport = () => {
  ...
  a.click()
  // 延后撤销：Firefox 在同 tick 内撤销会取消刚触发的下载
...
const handleImport = (e: Event) => {
  ...
  // 与书签 HTML 导入一致：限制文件大小，避免超大 JSON 被整读进内存
...
// 拆分子组件共享断点样式：样式按需复制，改一处需同步另两处（Toolbar/CategoryPanel/ItemsPanel）。
.toolbar-card {
  display: flex;
